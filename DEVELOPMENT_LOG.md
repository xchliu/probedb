...
## 2026-09-24（周四：状态检查 — 本周计划待更新）

### 状态
- **测试**: 196 passed, 0 failed ✅
- **本周任务**: 未分配（周计划停留在09-18，本周无新任务分解）
- **阻塞**: Hermes 接入被2个待确认事项阻塞（9/1起）

### 待确认事项（阻塞推进）
| 日期 | 事项 | 状态 |
|------|------|:----:|
| 09-01 | Hermes 接入切入点：直接改 hermes-agent 内部存储后端 vs 先做独立 probedb 存储工具？ | ⚠️ 待确认 |
| 09-01 | C ABI 选型结论确认；接口定义已完成（src/ffi.rs + bridge/probedb.py） | ⚠️ 待确认 |
...
- **多列 ORDER BY 静默忽略第二排序键**：`ORDER BY pinned DESC, created_date ASC` 只按 `pinned` 排序，`created_date` 被无声丢弃（写 demo 时发现）。静默忽略同样是"静默错误答案"族，建议与上一条合并为一次"静默降级排查"任务
